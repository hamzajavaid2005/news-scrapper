...
export const checkDuplicate = inngest.createFunction(
    ...
    async ({ event, step, logger }) => {
        ...
        logger.info(
            `[${getTimestamp()}] 🔄 Checking duplicates for article: ${articleId}`
        );

        // Step 1: Check for similar articles using embedding from DB
        const duplicateResult = await step.run(
            ...
            async () => {
                ...
                if (!hasEmbedding) {
                    logger.info(
                        `[${getTimestamp()}] ⚠️ No embedding available, skipping duplicate check`
                    );
        ...
        `;
                    ...
                    if (!embeddingResult[0]?.embedding) {
                        logger.info(
                            `[${getTimestamp()}] ⚠️ No embedding in DB, skipping duplicate check`
                        );
                        return { isDuplicate: false, article };
                    }
                    ...
                    if (similar.length > 0 && similar[0].url !== article.url) {
                        ...
                        logger.info(
                            `[${getTimestamp()}] 🔄 Found duplicate (${(duplicate.similarity * 100).toFixed(0)}%): ${article.title?.substring(0, 40)}...`
                        );
                        return {
                            isDuplicate: true,
                    ...
                    logger.info(
                        `[${getTimestamp()}] ✓ Article is unique: ${article.title?.substring(0, 40)}...`
                    );
                    return { isDuplicate: false, article };
                } catch (error) {
                    logger.warn(
                        `[${getTimestamp()}] ⚠️ Duplicate check failed: ${error.message}`
                    );
                    return { isDuplicate: false, article };
                }
            }
        ...
        if (duplicateResult.isDuplicate) {
            ...
            await step.run("mark-as-duplicate", async () => {
                ...
                console.log(
                    `🔄 [${getTimestamp()}] [${sourceName}] DUPLICATE (${(duplicate.similarity * 100).toFixed(0)}%): ${duplicateResult.article.title?.substring(0, 40)}...`
                );
                console.log(
                    `   └─ Similar to: ${duplicate.title?.substring(0, 45)}...`
                );
        ...
        logger.info(
            `[${getTimestamp()}] 📤 Dispatched save for unique article: ${articleId}`
        );

        return {
            message:
                "Duplicate check passed - article is unique and will be saved.",
            status: "unique",
            articleId,
